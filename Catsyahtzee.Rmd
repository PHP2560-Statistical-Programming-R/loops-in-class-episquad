...
```
#bubblesort is a function that is applied to x if the length of x is less 2 then it will return x. For the last value in length of x to the second row then execute the next loop. The following loop is for the first unit in row 1 to the next to last then execute the next loop. If the first value in x is greater than the first value plus one then swap the pair by saving the first, adding one to it and placing it back into the first +1. Return x, allows it to print the value. 
...
if (x[1]==x[5]){
  hand="Yahtzee"
    ...
    hand="Large Straight"
}  else if (x[4]==x[3]+1 & x[3]==x[2]+1 & x[2]==x[1]+1 | x[5]==x[4]+1 & x[4]==x[3]+1 & x[3]==x[2]+1) {
    hand="Small Straight"
} else hand="Chance"
hand
...
sim <- matrix(, nrow=100000, ncol=5)

for (i in 1:nrow(sim))
  {
  sim[i,] <- sample(1:6, size=5, replace=T)
}
bubblesort <- function(sim) {
# x is initially the input vector and will be
# modified to form the output
if (length(sim) < 2) return (sim)
# last is the last element to compare with
for(last in length(sim):2) {
for(first in 1:(last - 1)) {
if(sim[first] > sim[first + 1]) { # swap the pair
save <- sim[first]
sim[first] <- sim[first + 1]
sim[first + 1] <- save
}
}
}
...
library(dplyr)
prob<- x %>% group_by(hand) %>% summarise (n = n()) %>%  mutate(Probability = n / sum(n)) 
prob
```
--------------------------------
Hand              Probability
