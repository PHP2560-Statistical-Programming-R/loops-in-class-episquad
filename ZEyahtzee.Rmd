...
```
# bubble_sort is calling on a previously named group called vec. Inside bubble_sort it is defining a value called "no_passes" which equals 0. The while function executes a loop function while the identified value is true. In this case while the value is 1 the following loop executes. The loop is if no_swaps=0 then for every value j in the first value of the matrix to the length of vec minus 1 minus the value of no_passes (value=0). If these conditions are satisfied than another loop executes that states for every value j in vec that is one unit greater than the previous j then another loop executes. Renaming each j value of vec to s, and the j value of vec equals the same value moved over one unit which is assigned again to s creating a loop. Then no_swaps is assigned a new value of 0 +1. No_passes then adds one more unit to 0 if no_swaps equals 0. Break then does not return a value. 

# Starting from the inside loop for every value j in vec that is one unit greater than the previous j then executes
...
if (x[1]==x[5]){
  hand="Yahtzee"
    ...
    hand="3ofKind"  
}  else if (x[1]==x[2]-1 & x[2]==x[3]-1 & x[3]==x[4]-1 & x[4]==x[5]-1) {
    hand="Large Straight"
}  else if (x[1]==x[2]-1 & x[2]==x[3]-1 & x[3]==x[4]-1 | x[2]==x[3]-1 & x[3]==x[4]-1 & x[4]==x[5]-1  ) {
    hand="Small Straight"
} else {hand="Chance"}
```
...
x<-as.data.frame(x)
x$hand<-0
for (i in 1:nrow(x)) {x$hand[i]<-hand(x[i,])}
library(dplyr)
x<- x %>% group_by(hand) %>% count() %>% mutate(Probability=(n/sum(n)))
```
